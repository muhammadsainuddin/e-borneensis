...
// === FUNGSI BARU UNTUK RESET DATA ===
function resetData() {
  if (confirm('Amaran: Ini akan memadam semua rekod offline dan memuatkan semula data dummy. Teruskan?')) {
    localStorage.removeItem('encounters')
    alert('Data berjaya di-reset. Aplikasi akan dimuat semula.')
...
<template>
  <div class="absolute inset-0 w-full h-full overflow-y-auto bg-[#f4f6f8] px-4 pt-8 pb-32 font-sans text-left">
    ...
    <button @click="resetData" class="w-full bg-white rounded-[24px] shadow-sm border border-yellow-100 p-4 mb-5 flex items-center justify-center hover:bg-yellow-50 transition-colors active:scale-95">
      ...
      <span class="text-sm font-bold text-yellow-600 tracking-wide">RESET DUMMY DATA</span>
    </button>
    ...
    <Teleport to="body">
      <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showEditProfile" class="fixed inset-0 z-[9999] flex items-end justify-center font-sans text-left">
          <div @click="showEditProfile = false" class="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>
          <Transition enter-active-class="transition duration-300 ease-out translate-y-full" enter-from-class="translate-y-full" enter-to-class="translate-y-0" leave-active-class="transition duration-200 ease-in translate-y-0" leave-from-class="translate-y-0" leave-to-class="translate-y-full">
            <div v-if="showEditProfile" class="relative bg-white w-full max-w-lg rounded-t-[32px] shadow-2xl flex flex-col p-6 pb-10">
      ...
      <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showSettings" class="fixed inset-0 z-[9999] flex items-end justify-center font-sans text-left">
          <div @click="showSettings = false" class="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>
          <Transition enter-active-class="transition duration-300 ease-out translate-y-full" enter-from-class="translate-y-full" enter-to-class="translate-y-0" leave-active-class="transition duration-200 ease-in translate-y-0" leave-from-class="translate-y-0" leave-to-class="translate-y-full">
            <div v-if="showSettings" class="relative bg-[#f4f6f8] w-full max-w-lg rounded-t-[32px] shadow-2xl flex flex-col p-6 pb-10">
              <div class="w-12 h-1.5 bg-gray-300 rounded-full mx-auto mb-6"></div>
      ...
      <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showAbout" class="fixed inset-0 z-[9999] flex items-end justify-center font-sans text-left">
          <div @click="showAbout = false" class="absolute inset-0 bg-black/60 backdrop-blur-md"></div>
          <Transition enter-active-class="transition duration-300 ease-out translate-y-full" enter-from-class="translate-y-full" enter-to-class="translate-y-0" leave-active-class="transition duration-200 ease-in translate-y-0" leave-from-class="translate-y-0" leave-to-class="translate-y-full">
            <div v-if="showAbout" class="relative bg-white w-full max-w-lg rounded-t-[32px] shadow-2xl flex flex-col p-8 pb-10 text-center items-center">
